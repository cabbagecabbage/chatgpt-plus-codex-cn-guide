...
| [从注册开始 →](#三注册-gmail) | [付款前必读 →](#五付款前必须知道的事) | [看看模型怎么选 →](#十二codex-模型与推理强度选择) | [查看工具（中文）→](https://github.com/cabbagecabbage/codex-model-slider/blob/main/README.zh-CN.md) |
...
| 准备美区 Apple ID 与礼品卡 | [第六章：注册美区 Apple ID](#六注册美区-apple-id) |
| 通过美区 Apple ID 订阅 Plus | [第七章：通过美区 Apple ID 为 ChatGPT 添加订阅](#七通过美区-apple-id-为-chatgpt-添加订阅) |
| Codex 手机号验证与接码 | [第八章：Codex 短信接码](#八codex-短信接码) |
| 使用 CC Switch 切换 Codex 登录状态 | [第九章：使用 CC Switch 切换登录状态，避免反复退出登录](#九使用-cc-switch-切换登录状态避免反复退出登录) |
| 查看楼主使用记录 | [附录：楼主使用记录（持续更新）](#附录楼主使用记录持续更新) |
| 使用楼主自用中转 | [第十章：楼主自用中转](#十楼主自用中转) |
| 比较 Codex 周额度和中转站的性价比 | [第十一章：Codex 周额度理论倍率与中转站性价比对比](#十一codex-周额度理论倍率与中转站性价比对比) |
| 选择 Codex 模型、推理强度和升档路线 | [第十二章：Codex 模型与推理强度选择](#十二codex-模型与推理强度选择) |
...
> 这会导致什么问题？举个例子：
>
> 1. **第一次，在这部手机上：** App Store（“媒体与购买项目”）登录 **Apple ID A**，ChatGPT App 登录**账号 1**。你在 ChatGPT App 中点击订阅并完成 Apple 内购付款，账号 1 获得订阅，Apple ID A 与账号 1 绑定。
> 2. **第二次，还是同一部手机：** App Store 仍登录 **Apple ID A**，但 ChatGPT App 已切换为**账号 2**。你在 ChatGPT App 中再次点击订阅并付款，以为这次是在给账号 2 充值。
> 3. **实际结果：** 即使付款成功，订阅仍关联到**账号 1**，而不是 ChatGPT App 当前登录的**账号 2**。因为付款用的仍是 Apple ID A，它绑定的还是账号 1。
>
> **换句话说，切换 ChatGPT 登录账号，并不会改变这个 Apple ID 的订阅归属。**
>
> 楼主踩过的坑是：**账号 1 已经删除，再用 Apple ID A 给账号 2 订阅，付款仍关联到已删除的账号 1，账号 2 依然没有订阅。** 删除原账号也没有解除这层绑定，所以第一次付款就要选好准备长期使用的 ChatGPT 账号。

> [!WARNING]
> **如果真的充错了，可以尝试申请退款。** 楼主的经历是：第一次直接获批，没有遇到额外条件；但第二次申请和后续复核都被拒了。**所以不能指望退款来反复兜底，第一次付款就要选对账号。** 具体经过和客服截图见第七章。

### 2. Codex 能登录一次，不代表以后不再验证

> [!IMPORTANT]
> **真正的坑是二次验证：以后再次要求向原手机号发验证码时，你还能不能收到？**
>
> 临时接码只能解决当次验证，号码过期后，再次验证就可能卡住。准备长期使用 Codex，优先准备一个**平台接受、自己能长期收验证码的海外手机号**，首次绑定后就保留好。

楼主对触发原因的判断是：机场多人共用的出口 IP、数据中心 IP 等网络环境，可能更容易引起登录风控；独享住宅 IP（俗称“纯净家宽”）可能少遇到验证。但是否验证不能只看 IP，家宽也不能保证首次或以后都免验证。

因此，不要因为这次没要求验证码，就认为以后也不会要。付款前先尝试登录，出现验证时考虑好号码的长期可用性；临时接码和后续申诉的经历放在第八章。

---

## 六、注册美区 Apple ID

本章介绍美区 Apple ID 和礼品卡的准备。已有美区 Apple ID 就不用重新注册；两项都准备好了，就继续下一章的订阅步骤。
...
## 七、通过美区 Apple ID 为 ChatGPT 添加订阅
...
## 九、使用 CC Switch 切换登录状态，避免反复退出登录
...
目前楼主主要使用 [zz-api.cc.cd](https://ww.zz-api.cc.cd/register?aff=TGTMUXRZFVTR) 的 `0.10 倍率`分组。也有更低倍率的分组，但稳定性不太好。本文也对中转的整体性价比进行了分析，详见[第十一章：Codex 周额度理论倍率与中转站性价比对比](#十一codex-周额度理论倍率与中转站性价比对比)。
...
> **你真的有这么大用量吗？**
>
> 模型和推理强度会影响额度消耗，也要结合完成任务的速度来选择。此前按 Luna 降价后的口径估算，Luna Max 在所选 9 项评测中的总成本比 Sol Medium 低约 83%；这不等于实际订阅额度也会节省同样的比例。楼主目前按任务选择 Luna Max、Sol High 或 Astra Medium，见[第十二章：Codex 模型与推理强度选择](#十二codex-模型与推理强度选择)。
...
最近常用的档位调整为 Luna Max、Sol High 和 Astra Medium：简单需求用 Luna 节省额度，有测试和验收机制的工程开发优先用 Sol，方案、机制、架构和 Skill 设计更倾向于直接用 Astra。体感上 Astra 完成任务最快、也更容易抓住重点，但额度消耗更快；少量需要人参与思考的设计讨论，楼主愿意为更好的判断力花这部分额度。以上是个人使用感受，详细说明见[第十二章](#十二codex-模型与推理强度选择)。
...
本周楼主主要使用官方 Plus。中转线路倍率不太稳定，基本在 `0.07～0.1` 之间波动。楼主每次都能在额度刷新前稳定用完周额度，而这个月截至当时已经额外重置 10 次。7 月最终的有效额度和成本，按第十一章的实际时间线算法计算。目前中转主要用于 Plus 额度用完后的兜底。

不过，楼主的使用情况不一定适用于所有人。如果你的 Token 用量较少、无法用完 Plus 额度，中转可能仍然更划算。建议参考[第十一章：Codex 周额度理论倍率与中转站性价比对比](#十一codex-周额度理论倍率与中转站性价比对比)，按照自己的实际用量估算后再决定。
