...
7 月 22 日早上 Tibo 又重置了一次额度。根据 [Codex Resets](https://codex-resets.com/) 的记录，2026 年 7 月截至目前已经额外重置 9 次，因此本月可按 13.5 倍周额度计算：

```text
(9 次额外重置 + 4.5 倍正常月额度) × 150 美元
= 13.5 × 150 美元
= 2,025 美元 API 等值用量
```
...
<p align="center"><em>截至 7 月 22 日，Codex Resets 记录的额度重置情况</em></p>

中转常说的“倍率”，可以简单理解为：购买 `1 美元官方 API 等值用量`需要支付多少人民币。用 Plus 月费除以本月全部可用额度，就能得到本月的临界倍率：

```text
140 元 ÷ 2,025 美元 API 等值用量
≈ 0.0691 元/美元 API 等值
```

也就是说，2026 年 7 月 Plus 的临界倍率约为 `0.069`，可以简单记作 `0.07`：

| 中转倍率 | 购买 2,025 美元等值用量的成本 | 只看 Token 成本的结论 |
|---:|---:|---|
| `0.03` | 60.75 元 | 中转更划算 |
| `0.06` | 121.50 元 | 中转更划算 |
| `0.069` | 约 140 元 | 接近临界点 |
...
| 9 次（本月目前） | 2,025 | `0.0691` | `0.0346` |
| 10 次 | 2,175 | `0.0644` | `0.0322` |

如果一个月没有额外重置，Plus 的临界倍率约为 `0.207`；额外重置 3～5 次时约为 `0.124～0.098`。本月比较特殊，到 22 日已经额外重置 9 次，临界倍率降到了 `0.069`，后面如果继续重置还会更低。每个月实际的临界倍率，还是要看 Tibo 大善人的心情～

### 20x Pro 的理论临界倍率

表中的 20x Pro 按“月费是 Plus 的 10 倍、周额度是 20 倍”计算，因此理论临界倍率正好是 Plus 的一半。本月对应 `0.0346`：低于它时中转更划算，高于它时 Pro 理论上更划算。
