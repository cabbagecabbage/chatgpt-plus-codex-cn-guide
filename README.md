...
**触发原因：** 如果你用的是机场 IP 或商用集群 IP，而非纯净家宽，就很有可能被要求手机号验证，也就是接收 OpenAI 发来的短信验证码。
...
3. **两者都没有：** 只能用接码网站收一次临时验证码，同时祈祷以后不要触发二次验证。具体接码方法见第八章。

> [!WARNING]
> **用临时号码验证后，尽量保持登录状态，不要退出登录。** 退出后重新登录可能触发二次验证；需要切换账号时，用 **CC Switch** 切换，具体方法见第九章。
